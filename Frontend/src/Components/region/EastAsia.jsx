...
import eastAsia from "../../assets/images/eastasia.png";
const styles = `
.sa-root {
  font-family: 'Inter', sans-serif;
  background-image: url(${eastAsia});
...
export default function EastAsia() {
  ...
  const loadData = () => {
    const stored = localStorage.getItem("weatherData_East Asia");
    if (!stored) {
      setLoading(true); // skeleton reste affiché
      return;
    }
    const eas = JSON.parse(stored);
    const { weather, forecast, image, windy_embed } = eas;
