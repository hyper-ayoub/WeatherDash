...
.sa-root {
  ...
  background-image: url(${centralAsia});
...
export default function CentralAsia() {
  ...
  const loadData = () => {
    const stored = localStorage.getItem("weatherData_Central Asia");
    if (!stored) {
      setLoading(true); 
      return;
    }
    const ce = JSON.parse(stored);
    const { weather, forecast, image, windy_embed } = ce;
